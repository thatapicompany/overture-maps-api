...
export class PlacesService {
    ...
    async getPlaces(query: GetPlacesDto):Promise<{ results: Place[]; totalCount: number }> {
        ...
        const { lat, lng, radius,  country, min_confidence, brand_wikidata,brand_name,categories,limit, page = 0, source, operating_status, taxonomy, has_contact, name } = query;
        ...
        const cacheKey = buildCacheKey('get-places', { lat, lng, radius, country, min_confidence, brand_wikidata, brand_name, categories, limit, operating_status, taxonomy, has_contact, name, page: page > 0 ? page : undefined });
        ...
        if (!cached?.results) {
          cached = await this.bigQueryService.getPlacesNearby(lat, lng, radius, brand_wikidata, brand_name, country, categories, min_confidence, limit, undefined, operating_status, taxonomy, page, has_contact, name);
